...
const SERVER_BASE_URL = "http://192.168.0.19:3000";
...
export default function SettingsScreen() {
  ...
  const [libreMessage, setLibreMessage] = useState("");
  ...
  async function checkLibre() {
    await checkEngine("libre", setLibreStatus, setLibreMessage);
  }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>설정 / 연결 상태</Text>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Libre 번역 서비스</Text>
          <StatusBadge status={libreStatus} />
        </View>

        {libreStatus === "checking" ? (
          <View style={styles.rowCenter}>
            <ActivityIndicator />
            <Text style={{ marginLeft: 8 }}>Libre 테스트 중입니다...</Text>
          </View>
        ) : (
          <Text style={styles.message}>
            {libreMessage || "아직 Libre 상태를 확인하지 않았습니다."}
          </Text>
        )}

        <Text style={styles.helperText}>
          • LibreTranslate 서버(LIBRE_ENDPOINT)가 응답하는지 확인합니다.{"\n"}
          • 직접 구축한 Libre 서버 또는 공개 Libre 인스턴스 상태에 따라 달라집니다.
        </Text>

        <TouchableOpacity style={styles.engineBtn} onPress={checkLibre}>
          <Text style={styles.engineBtnText}>Libre만 다시 확인</Text>
        </TouchableOpacity>
      </View>
